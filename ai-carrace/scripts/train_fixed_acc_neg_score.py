...
name = "carai_fixed_acc_neg_score_v1"
...
class CarGenome(training.Genome):
    ...
    def run_evaluation(self, generation: int = None):
        ...
        for timestep in range(timestepamount):
            if not self.step(timestepamount):
                break

        self.obj.score -= (timestep * 0.001)
...
if training_mode:
    ...
    trainer.population_size = 200
    trainer.learning_rate_base = 0.2
    trainer.learning_rate_factor = 0.99
    trainer.mutation_chance = 0.75
    trainer.setup_auto()
